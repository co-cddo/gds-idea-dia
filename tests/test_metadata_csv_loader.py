"""Tests for dia.metadata.csv_loader — S3 CSV metadata loading."""

import boto3
import pytest
from moto import mock_aws

from dia.metadata.csv_loader import find_latest_key, load_csv_from_s3, load_latest_csv_from_s3
...
def test_loads_empty_csv(s3_client):
    s3_client.put_object(Bucket=BUCKET, Key="empty.csv", Body=b"department,alb,filenames\n")

    rows = load_csv_from_s3(BUCKET, "empty.csv", s3_client=s3_client)
