...
extractors (offloaded to threads), writes output via an injectable writer,
and records success in the ledger.
...
from dia.metadata import MetadataProvider
...
class TextExtractionRunner:
    """Orchestrates Stage 1: download + extract text + write output.
        ...
        for batch_idx, batch in enumerate(batches, start=1):
            ...
            for ref, result in zip(batch, batch_results):
                if isinstance(result, Exception):
                    ...
                    await self._write_output(ref, result)
    ...
    async def _process_batch(self, batch: list[DocumentReference]) -> list[str | Exception]:
    ...
    async def _write_output(self, ref: DocumentReference, text: str) -> None:
        """Build the output document and hand it to the configured writer."""
        ...
        output = TextExtractionOutput(
            ...
            text=text,
            chars=len(text),
            metadata=self._resolve_metadata(ref),
            extracted_at=datetime.now(tz=UTC).isoformat(),
            code_version=version("dia"),
        )
        await asyncio.to_thread(self._writer.write, output_key, asdict(output))

    def _resolve_metadata(self, ref: DocumentReference) -> dict:
        """Look up document metadata for enrichment, if a provider is configured."""
        if self._metadata is None:
            return {}

        meta = self._metadata.get_metadata(ref.key)
        if meta is None:
            return {}
